...
- (void)stroke{
    ...
    for (NSDictionary *dict in _dataItems) {
        NSString *string = [NSString stringWithFormat:@"%@",dict[@"person"]];
    ...
    for (int i = 0; i < _dataArr.count; i++) {
        ...
        end = [_dataArr[i] floatValue] / _total + start;
        ...
        NSDictionary *dict = _dataItems[i];
